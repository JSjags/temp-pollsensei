...
const PublicResponse = () => {
  ...
  const question = typeof params?.id === 'string' && params.id.startsWith('ps-') ? psId : psShortUrl;

  const handleSubmitResponse = async () => {
 
    const formattedAnswers = question?.data?.sections[currentSection]?.questions.map((item: any, index: number) => {
      if (item.question_type === "multiple_choice" || item.question_type === "single_choice") {
        return {
          question: item.question,
          question_type: item.question_type,
          selected_options: [selectedOptions[index]]
        };
      } else if (item.question_type === "comment" || item.question_type === "long_text" || item.question_type === "short_text") {
        return {
          question: item.question,
          question_type: item.question_type,
    ...
    if(!formattedAnswers || question?.data?.settings?.collect_email_addresses === true && respondent_email ==='' || question?.data?.settings?.collect_name_of_respondents === true && respondent_name === '' ){
  ...
  return (
<div
      className={`${''} flex flex-col gap-5 w-full lg:px-16 `}
    >
      { question?.data &&
       
      <div className={`${question?.data?.theme} flex justify-center  items-center px-5 mx-auto gap-10 lg:w-[80%]`}>
        <div className={` flex flex-col overflow-y-auto max-h-screen custom-scrollbar`}>
          ...
          <div className="bg-white rounded-lg w-full my-4 flex gap-2 px-11 py-4 flex-col ">
            <h2 className="text-[1.5rem] font-normal" style={{fontSize:`${question?.data?.headerText?.size}px`, fontFamily:`${question?.data?.headerText?.name}` }}>{question?.data?.topic}</h2>
            <p style={{fontSize:`${question?.data?.body_text?.size}px`, fontFamily:`${question?.data?.body_text?.name}` }}>{question?.data?.description}</p>
          </div>
          ...
          <div className='flex flex-col gap-2 w-full bg-white px-11 py-4 rounded-lg mb-4'>
            {
              question?.data?.settings?.collect_email_addresses && (
                ...
                <label htmlFor="full name" className='pl-5'>Full name <sup className='text-red-700 text-sm'>*</sup></label>
                <input type="text" className='border-b-2 rounded-b-md ring-0 shadow-sm active:border-none focus:border-none py-1 px-4 outline-none ' required onChange={(e)=>setRespondent_name(e.target.value)} value={respondent_name} />
              </div>
              )
            }
            {
              question?.data?.settings?.collect_name_of_respondents && (
                <div className='flex flex-col w-full'>
                <label htmlFor="full name" className='pl-5'>Email <sup className='text-red-700 text-sm'>*</sup></label>
                <input type="email" className='border-b-2 rounded-b-md ring-0 shadow-sm active:border-none focus:border-none py-1 px-4 outline-none ' required onChange={(e)=>setRespondent_email(e.target.value)} value={respondent_email} />
              </div>
              )
            }
          </div>
