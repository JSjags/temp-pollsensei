...
const UserRegistry: React.FC = () => {
  ...
  const { data, isLoading, error, refetch } = useUserRegistryQuery({
  ...
  const handleFilterApply = (filterType: string, selectedOptions: string[]) => {
    console.log(`Applied ${filterType} Filter:`, selectedOptions);
  };
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-">
          ...
          <DropdownFilter
            ...
            onApply={(selected) => handleFilterApply("Subscription", selected)}
          />
          <DropdownFilter
            title="Account Type"
            options={options.accountType}
            onApply={(selected) => handleFilterApply("Account Type", selected)}
          />
          <DropdownFilter
            title="Location"
          ...
          />
          <button className="text-red-500 bg-white border-gray-300 py-2 px-4 text-sm border rounded-r-full">
            Reset Filter
          </button>
        </div>
        <input
