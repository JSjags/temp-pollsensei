...
export function FirstStep() {
  ...
  const { unrestrictedBalance = 0 } = userBalance || {};
  ...
  const validate = () => {
    ...
    } else if (quantityNum < threshold) {
      newErrors.quantity = `Amount must be at least ${threshold}.`;
    }
  ...
  const handleSubmit = () => {
    if (unrestrictedBalance === 0) return;
    if (!validate()) return;
    setStep("checkout");
  };
          ...
          <div className="mt-4 flex items-center w-full justify-between h-[54px] border rounded-md bg-muted/50">
            <div className="h-[54px] flex items-center justify-center min-w-[90px] border-r px-3">
              <p className="text-sm text-muted-foreground">
                {locationLoading ? (
                  <Skeleton className="h-5 min-w-[120px] w-full" />
                ) : (
                  <p className="text-sm text-muted-foreground">
            ...
            {locationLoading ? (
              <Skeleton className="h-5 w-[60%] mr-1" />
            ) : (
              <input
                type="text"
                value={formatNumber(Number(coinAmount))}
                readOnly
                className="h-[54px] flex-1 pl-2.5 bg-transparent outline-none text-muted-foreground select-none"
              />
            )}
          </div>
          ...
          <div className="mt-4 rounded-[10px] bg-[#F7F8FB] w-full py-1.5 px-2.5">
            {isLoading ? (
              ...
              <p className="text-[#898989] text-xs">
                ...
                {isNigeria
                  ? `₦${(
                      data?.conversion_rate?.NGN * data?.convertible_amount
                    ).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}`
                  : `$${(
                      data?.conversion_rate?.USD * data?.convertible_amount
                    ).toLocaleString(undefined, {
                      minimumFractionDigits: 2,
                      maximumFractionDigits: 2,
                    })}`}
