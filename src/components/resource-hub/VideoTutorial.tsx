    ...
    <div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
        {videoIsLOading ? (
          ...
          videoData?.data?.map((article: any, index: number) => {
            const media = article?.media?.[0];
            const isImage = media?.type?.startsWith("image/") && !!media?.url;
            const isVideo = media?.type?.includes("video") && !!media?.url;
            ...
            return (
              <div
                ...
                tabIndex={0}
                aria-label={article?.title || "View video tutorial"}
              >
                <Link
                  href={routes.SINGLE_ARTICLE_PAGE(article?.slug)}
                  tabIndex={-1}
                >
                  <div className="w-full flex justify-center items-center bg-gray-50 !p-0 !m-0 relative">
                    {isImage ? (
                      <div className="w-full h-40 relative mt-0">
                        <Image
                          src={media.url}
                          alt={article?.title || "Video tutorial image"}
                          fill
