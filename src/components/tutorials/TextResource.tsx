...
const TextResources = (): JSX.Element => {
  ...
  const fetchTutorials = async () => {
    const response = await axiosInstance.get(
      `/tutorial?page=${currentPage}&page_size=${PAGE_SIZE}&filter_by=text`
    );
    return response;
  };

  const { data, isLoading, refetch, isRefetching } = useQuery({
    queryKey: ["text-tutorials", currentPage],
  ...
  return (
    <main className="p-5">
      {isLoading || isRefetching ? (
        ...
        <EmptyState
          title="No text resource found"
          description="We couldn't find any text resource matching your search or filters. Try adjusting your criteria or check back later for new content."
          icon={<Newspaper className="w-12 h-12 text-purple-400 mx-auto" />}
        />
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
