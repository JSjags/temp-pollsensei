...
const DeleteFaq: React.FC<DeleteFaqProps> = ({ onClose, openModal, onDelete, isLoading }) => {
  return (
    <Modals title="" openModal={openModal} modalSize="lg" onClose={onClose}>
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-center text-[#838383] text-[14px]">
          Are you sure you want to delete this? Note that this cannot
          be undone.
        </p>
        <div className="mt-3 flex items-center justify-between w-full">
          <ButtonOutline label="No, cancel" onclick={onClose} />
          <ButtonDelete label={isLoading ? "Deleting": "Delete"} onclick={onDelete}/>
        </div>
      </div>
    </Modals>
  );
