...
const LikertScaleQuestion: React.FC<LikertScaleQuestionProps> = ({
  ...
  options,
  questionType,
  scale_value = [], // Default to an empty array
  EditQuestion,
  onChange,
  index,
  DeleteQuestion,
  ...
  // Sync scale_value with selectedOption
  useEffect(() => {
    if (scale_value.length > 0) {
      setSelectedOption(scale_value[0]); // Use the first value from the array
    }
  }, [scale_value]);
