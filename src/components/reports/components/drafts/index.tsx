...
} from "@/components/reports/queries/usePostOnboard";
      ...
      uploadMedia(
        ...
        },
        {
          onSuccess: (res: any) => {
            ...
            // 3. { url: string }
            const url =
              res?.data?.url ?? res?.media?.[0]?.url ?? res?.url ?? null;
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="space-y-2">
          ...
          <Select
            ...
            disabled={categoriesLoading || noCatData}
          >
            <SelectTrigger>
              <SelectValue
                placeholder={
...
//     useReportInterests(); // <-- NEW
// const { upsertDraft, getDraft, setActiveReportId } = useReportDraftStore();
...
//     {
//       label:
//         form.title?.trim() || report?.title || titleFromQuery || "Untitled",
//     },
//   ];
...
export function ReportDraftPage({
  ...
  const safeDescription = report?.description ?? "";
  const safeCategory = report?.category ?? "";
  const safeInterests = (report?.interests ?? []) as string[];
  const safeThumb = report?.thumbnailUrl ?? null;
  const safeBody = report?.body ?? "";
  const reportId = report?.id ?? routeReportId ?? "";
  const titleFromQuery = searchParams.get("title");

  // -------------------------------------------------------------------------
  // Local form state with persistence
  // -------------------------------------------------------------------------
  const [form, setForm] = React.useState<ReportFormState>({
    title: safeTitle,
    description: safeDescription,
  ...
  // Load existing draft from store
  React.useEffect(() => {
    if (!reportId) return;
    setActiveReportId(reportId);
    const stored = getDraft(reportId);
    if (stored) {
      setForm({
        title: stored.title || safeTitle,
        description: stored.description || safeDescription,
        category: stored.category || safeCategory,
        interests: stored.interests || safeInterests,
        thumbnailUrl: stored.thumbnailUrl || safeThumb,
      });
      setCurrentContent(stored.body || safeBody);
    } else {
      upsertDraft(reportId, {
        title: safeTitle,
        description: safeDescription,
        category: safeCategory,
        interests: safeInterests,
  ...
  const updateForm = (partial: Partial<ReportFormState>) => {
    setForm((prev) => {
      ...
      emitMeta(next);
      if (reportId) {
        upsertDraft(reportId, { ...next, body: currentContent });
      }
      return next;
    });
  };
  ...
  const handleContentChange = React.useCallback(
    (body: string) => {
      setCurrentContent(body);
      if (reportId) upsertDraft(reportId, { body });
      if (!onContentChange) return;
      setSaving(true);
      if (saveTimerRef.current) clearTimeout(saveTimerRef.current);
      saveTimerRef.current = setTimeout(() => {
    ...
    [onContentChange, autosaveDelayMs, reportId, upsertDraft]
  );
